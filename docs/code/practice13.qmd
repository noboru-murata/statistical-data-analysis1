...
library(tidyverse)
library(broom)       # tidy(), gloance(), augment() を利用する
...
toy_lm |> tidy()    # 推定された係数の情報
toy_lm |> glance()  # 推定に関するさまざまな情報
toy_lm |> augment() # 推定に用いられたデータの情報
```
...
tw_conf <-
    tw_lm |> 
    augment(newdata = tibble(solar = tw_data |> pull(solar) |>
                                 range() |> # 日射量の範囲を取得
...
```{r}
tw_data |>
    mutate(augment(tw_lm, interval = "confidence")) |>
    ggplot(aes(x = solar, y = temp)) +
    geom_point(aes(colour = factor(month))) + 
    geom_line(aes(y = .fitted), 
              colour = "royalblue") +
...
```{r}
tw_lm |> tidy() # 係数の情報をまとめたデータフレーム
tw_lm |> tidy() |> # solarの係数のt統計量とp値を抽出
    filter(term == "solar") |> select(statistic, p.value)
tw_lm |> tidy() |> _[2, c("statistic","p.value")]
tw_lm |> tidy() |> _[2,4:5]
tw_lm |> glance() # モデルの情報をまとめたデータフレーム
tw_lm |> glance() |> # F統計量に関する情報
...
bb_new_data <- # 作成したbody列に対する区間の計算
    tibble(bb_new_body,
           ...
           select(.fitted, .lower, .upper) |> exp() |>
           set_names(c("brain","c.lower","c.upper")), # 列名の指定
           augment(bb_lm,
                          newdata = bb_new_body,
                          interval = "prediction") |> # 予測区間
           select(.lower, .upper) |> exp() |>
           set_names(c("p.lower","p.upper"))) # 列名の指定
Animals |>
    rownames_to_column() |>
    ggplot(aes(body, brain)) + 
    geom_point(colour = "royalblue") +
