...
n <- 16 
p <- 0.6
my_random <- function(){ # Bernoulli分布をm個生成して合計
...
```{r}
my_data |> 
    as_tibble_col() |>              # tibbleに変換
    count(value) |>                 # 頻度の集計
    ggplot(aes(x = value, y = n)) + # 描画の基本設定
...
my_data |>
    as_tibble_col() |>
    count(value) |>
    set_names(c("値","観測値")) |>                     # 列名を日本語に変更
    mutate(観測値 = 観測値/sum(観測値),                  # 確率に変換
           理論値 = dbinom(値, size = n, prob = p)) |> # 理論値を追加
    pivot_longer(!値, values_to = "確率") |>           # long formatに変換
    ggplot(aes(x = 値, y = 確率, fill = name)) +       # 描画の基本設定
...
`base::table()` および `base::prop.table()` を利用する場合．
...
- $U_{1},U_{2}$ を $(0,1)$ 上の一様分布に従う独立な確率変数とする． このとき $$  \begin{align}
      X_{1}&=\sqrt{-2\log(U_{1})}\cos(2\pi U_{2}),\\
      X_{2}&=\sqrt{-2\log(U_{1})}\sin(2\pi U_{2})
    \end{align}$$ とおくと， $X_{1},X_{2}$ は独立かつともに標準正規分布に従う． これをグラフに描画して確認しなさい．
...
理論値は関数 `stats::dnorm()` で計算できるので， 関数 `ggplot2::geom_function()` を用いて重ね描きする．
...
```{r}
x <- replicate(mc, boxmuller()) # 2行 x mc列の行列が得られる
...
x |>
    t() |>                                 # 行列を転置
    as_tibble(.name_repair = "minimal") |> # tibbleに変換，列名の修正を指示
    set_names(paste0("x", 1:2)) |>         # 列名の書き換え
    ggplot(aes(x = x1, y = x2)) +
    geom_point(colour = "royalblue")
```
:::
...
mc <- 10000 # 実験回数を指定
my_data <-
    replicate(mc, my_random()) # 注で使うので保存しておく
my_data |>
    as_tibble_col() |>
    ggplot(aes(x = value)) +
    geom_histogram(aes(y = after_stat(density)), 
...
- $Z$ を標準正規分布に従う確率変数，$Y$ を自由度 $k$ の $\chi^2$ 分布に従う確率変数とし，$Z,Y$ は独立であるとする． このとき確率変数 $$  \begin{equation}
      \frac{Z}{\sqrt{Y/k}}
    \end{equation}$$ は自由度 $k$ の $t$ 分布に従うことを確認しなさい．
...
tibble(x = my_data) |>
    ...
    geom_function(fun = dt,
                  args = list(df = k),
                  colour = "red") +
    labs(title = bquote(paste(Z/sqrt(Y/k)," (",k==.(k),")"))) # "="を表示するには"=="とする
```
...
- $Y_{1},Y_{2}$ をそれぞれ自由度 $k_{1},k_{2}$ の $\chi^2$ 分布に従う独立な確率変数とする． このとき, 確率変数 $$ \begin{equation}
     \frac{Y_{1}/k_{1}}{Y_{2}/k_{2}}
   \end{equation}$$ は自由度 $k_{1},k_{2}$ の $F$ 分布に従うことを確認しなさい．
