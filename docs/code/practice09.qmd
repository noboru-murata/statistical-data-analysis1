...
その他の関数 `var()`, `sd()` などについても同様に計算すれば良い
...
#' 全ての組み合わせは for 文で実行可能 (以下の例では一部を表示)
for(item in c("temp","wind")){ # 項目を指定．c("temp","solar","wind") で全部指定
    for(func in c("mean")){ # 関数を指定．c("mean","var","sd") で全部指定
...
変数名を取り出すには以下のようにすればよい．

```{r}
which(tw_cor == max(tw_cor-diag(diag(tw_cor))), # 最大相関
      arr.ind = TRUE)                           # 配列の成分名を表示
```
...
::: column-margin
関数 `ggpairs()` のオプションを利用して選択することもできる．
...
負の最大相関を与える `temp` と `press` の散布図を描く．
...
気温の分位点を求める． 関数 `base::summary()` を利用する場合は以下のようになる．
...
```{r}
breaks <-          # 全データを用いて適切なビンを計算して固定する
    tw_data |>
    pull(temp) |>
    pretty(n = 50) # データの値域を50程度の切りの良い数字に分割
for(name in names(my_data)){
    gg <- my_data |>
        ggplot(aes(x = !!sym(name))) +
        geom_histogram(aes(y = after_stat(density)),
...
for(name in names(my_data)){
    gg <- my_data |>
        ...
        geom_vline(xintercept = tw_temp_summary[[name]],
                   colour = "red") +
        lims(x = range(tw_data |> pull(temp)), y = c(0,0.5)) + # 表示範囲を指定
        labs(x = "気温",
             title = paste("気温の", name, "の推定のばらつき"))
    print(gg)
}
...
tw_data |>
    count(wdir) |>
    slice_max(n) # n列が最大となる行を抽出する
tw_data |>       # filterを用いることもできる
    count(wdir) |>
    filter(n == max(n)) 
```
...
```{r}
#' 方向順に並べた文字列のベクトルを用意する
wdir_levels <- c("N","NNE","NE","ENE",
                 "E","ESE","SE","SSE",
                 "S","SSW","SW","WSW",
                 "W","WNW","NW","NNW")
tw_data |>
    mutate(wdir = factor(wdir, levels = wdir_levels)) |>
    count(wdir, .drop = FALSE) |> # 0も含めて16方位全て計算する
    ggplot(aes(x = wdir, y = n)) +
