...
mc_data <-
    replicate(mc, mc_trial(n)) # 1次元配列(ベクトル)が作成される
...
```{r}
mc_data <-
    replicate(mc, mc_trial(n, pval = FALSE)) |> # 1次元配列(ベクトル)が作成される
    as_tibble_col()
mc_data |>
    ggplot(aes(x = value)) +
...
mc <- 2000
replicate(mc, mc_trial(n)) |> # 2次元配列(行列)が作成される
    t() |> as_tibble() |>
...
ある番組の視聴率が2桁に達したかどうか知るために，n人にその番組を観たかどうか確認する． 確率変数 $$  \begin{equation}
    X_{i}=
    \begin{cases}
      1,&\text{番組を観た}\\
      0,&\text{番組を観ていない}
    \end{cases},\;
    i=1,2,\dotsc,n
  \end{equation}$$ を定義して，これを用いた検定を考えてみよ．
...
ばらつき方は大きく異なるが平均気温はばらつきと同程度の違いのように見える．
...
```{r}
x <- tw_data |> filter(month == 5) |> pull(temp)
y <- tw_data |> filter(month == 10) |> pull(temp)
t.test(x,y)
```
...
tw_data |>
    ...
    ggplot(aes(x = month, y = temp)) +
    geom_boxplot(fill = "azure") + # 色を変える
    geom_jitter(colour = "dodgerblue", width = 0.2) +
    labs(x = "月", y = "気温", title = "気温の比較")
```
