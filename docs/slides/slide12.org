...
# Time-stamp: <2026-07-02 10:20:57 mura>
:REVEAL:
...
- 分散分析における仮説の例
   #+attr_html: :class callout-tip
   #+attr_latex: :environment callout-tip
   #+begin_quote
...
- *\(F\) 検定*
   #+attr_html: :class callout-note
   #+attr_latex: :environment callout-note
   #+begin_quote
   \begin{equation}
     \text{検定統計量 : }
     F=\frac{s_1^2}{s_2^2}
...
- 検定問題
   ...
   #+attr_latex: :environment callout
   #+begin_quote
...
- 帰無仮説の下で次の事実が成り立つ
   -  \(SS_B,SS_W\) は独立
   -  \(SS_B/\sigma^{2}\) は自由度 \(p{-}1\) の \(\chi^2\) 分布に従う
   -  \(SS_W/\sigma^{2}\) は自由度 \(n{-}p\) の \(\chi^2\) 分布に従う
...
- 分散分析表の表示
   #+begin_src R :eval no :tangle no
     ...
     aov(formula, data) |> model.tables(type = "means")   # 平均値
     aov(formula, data) |> model.tables(type = "effects") # 効果(既定値)
   #+end_src
...
- 帰無仮説の下で次の事実が成り立つ
   -  \(SS_A,SS_E\) は独立
   -  \(SS_A/\sigma^{2}\) は自由度 \(a{-}1\) の \(\chi^2\) 分布に従う
   -  \(SS_E/\sigma^{2}\) は自由度 \((a{-}1)(b{-}1)\) の \(\chi^2\) 分布に従う
...
| \(\text{因子}B\) | \(b{-}1\)          | \(SS_B\) | \(\displaystyle\frac{SS_B}{b{-}1}\)          | \(F_B\)        | \(\displaystyle\int_{F_B}^\infty g(x)dx\)  |
|------------------+--------------------+----------+----------------------------------------------+----------------+-------------------------------------------|
