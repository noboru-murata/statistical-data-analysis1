...
# Time-stamp: <2026-05-06 15:31:04 mura>
:REVEAL:
...
* 講義の内容
:PROPERTIES:
:ID:       C97A9B41-24EF-4ED9-AEFE-EA9D664A76CE
:END:
- 可視化の重要性
- 基本的な描画
- いろいろな分布の視覚化
...
** tidyverse パッケージ
:PROPERTIES:
:ID:       A37A4209-00CE-4840-B92C-94B06F5D10BB
:END:
- データ操作とグラフィクスの拡張 (再掲)
...
** サンプルデータの説明
:PROPERTIES:
:ID:       9EF1CA72-C1A7-4AF9-993E-0BB3A26E6BDE
:END:
- ~jpdata[1-3].csv~ (再掲)
   #+begin_quote
...
- ~tokyo_covid19_2021.csv~ (~tokyo.zip~ の中)
   #+begin_quote
   - https://www.hokeniryo.metro.tokyo.lg.jp/kansen/info/corona/corona_portal (東京都)
   - データ項目
...
** 折線グラフ
:PROPERTIES:
:ID:       A42078C8-933A-499E-832C-56150234C52A
:END:
...
** データフレームの形の変更
:PROPERTIES:
:ID:       A34DEB5C-D8BD-4E58-A812-5C38DC331AAC
:END:
...
#+begin_src R :eval no :exports none
  ...
  pcr_data |>
    select(date,ciq,hc,ai) |> # 描画対象の列を抽出
...
:PROPERTIES:
:reveal_background: #fef4f4
:ID:       825AEC11-1758-4063-8C4A-3F17943901A8
:END:
...
** 対話型のグラフ
:PROPERTIES:
:ID:       B2CB215C-6EC6-4B64-9131-B1A846B36A3C
:END:
- ~ggplot2~ で描画したグラフは
...
- 以下のように日本語フォントを指定する必要がある
   #+begin_src R :eval no :tangle no
     if(Sys.info()["sysname"] == "Darwin") { # macOS か調べる
       ...
       #' geom_text/geom_label内で用いられる日本語フォントの指定
...
:PROPERTIES:
:reveal_background: #fef4f4
:ID:       D2717135-D54E-4622-AFB7-FBD84B682507
:END:
...
** 密度
:PROPERTIES:
:ID:       3CB3FD47-581F-4FA5-8E59-6009C0820177
:END:
...
:PROPERTIES:
:reveal_background: #fef4f4
:ID:       C1D936FE-2D7E-4191-88B9-4F4999CF280E
:END:
