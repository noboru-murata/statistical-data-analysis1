...
#+DATE: 
# Time-stamp: <2021-05-07 15:46:21 mura>
:reveal:
...
** 関数 ~curve()~ 
   ...
   - 関数 ~plot()~ にも同様の機能がある
     #+begin_src R :eval no
       plot(x, y=0, to=1, ...) # ... は"ベクトルの描画"と同じオプションが利用可能
     #+end_src
...
** 関数 ~plot()~
    - ベクトルデータの散布図を作成する (既出の機能)
      #+begin_src R :eval no
	plot(x, y=NULL, ...) #... は"ベクトルの描画"と同じオプションが利用可能
	## x: 1種類目のデータ c(x1,x2,...)
...
** ヒストグラム
   ...
   - 各棒グラフの矩形
     ...
     - 面積が区間に含まれるデータの個数に比例
   - データの分布の仕方を可視化するのに有効 \\
     (どのあたりに値が集中しているか, どの程度値にばらつきがあるかなど)
...
** 関数 ~boxplot()~
   ...
   - データフレーム ~x~ の
     変数 ~A~ (質的変数; 性別・植物の種類など)で変数 ~B~ を分類する場合
     #+begin_src R :eval no
       boxplot(B ~ A, data=x, ...) # A,B はデータフレームの列名
     #+end_src
