...
#+DATE: 
# Time-stamp: <2022-04-22 12:25:06 mura>
:REVEAL:
...
** ~if~ 文の例
- [[color:green][20220422が19で割り切れるか?]]
  #+begin_src R :tangle yes
...
- 条件Aが *真* である限りプログラムXを繰り返す
  #+begin_src R :eval no
    while(条件A) {プログラムX}
  #+end_src
- プログラムXは繰り返し必要な実行内容を記述し，
  終了するときに条件Aが満たされなくなるように書く
- /[[color:orange][~repeat~ 文というものもあるので調べてみよ]]/
...
- 前問で $X$ がベクトルの場合にはその平均を計算するように修正せよ \\
  /[[color:orange][関数 ~is.vector()~ が利用できる]]/
