...
# Time-stamp: <2024-04-26 16:06:07 mura>
:REVEAL:
...
- [[color:green][個別のグラフでの描画]]
  ...
  #+begin_src R :file figs/02-geomline-facetgrid.png :results graphics
    tw_data |> filter(month == 5) |> select(c(day, temp, solar)) |>
      ...
      facet_grid(vars(index)) # index ごとに行に並べる
  #+end_src
...
- [[color:green][湿度の情報を追加]]
  #+begin_src R :exports none
    #' 各軸を対数表示に変更
  #+end_src
  #+begin_src R :file figs/02-geompoint-size.png :results graphics :tangle yes
    tw_data |> filter(month %in% 7:9) |> 
      ggplot(aes(x = solar, y = temp, size = humid)) + # 湿度と点の大きさを対応
      geom_point(colour = "blue", shape = 19) + 
      labs(x = "solar radiation", y = "temperature")
  #+end_src
  #+name:   fig:02-geompoint-size
  #+attr_html: :width 50%
  #+attr_latex: :width 0.6\linewidth
  [[file:figs/02-geompoint-size.png]]
